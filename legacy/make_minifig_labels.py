...
if __name__ == '__main__':
	...
	filename_root = os.path.splitext(os.path.basename(minifig_data_file))[0]
	output_dir = libbrick.path_utils.get_output_dir()
	outfile = os.path.join(output_dir, "labels-" + filename_root + '.tex')
	pdffile = os.path.join(output_dir, "labels-" + filename_root + '.pdf')
	f = open(outfile, 'w')
	...
	f.close()
	BLwrap.close()
	print('\n\nmogrify -verbose -trim images/minifig_*.jpg; \nxelatex -output-directory "{0}" "{1}"; \nopen "{2}"'.format(output_dir, outfile, pdffile))
